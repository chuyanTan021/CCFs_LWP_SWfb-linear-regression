...
def LWP_obs_trends_2(data_Array_actual, data_Array_predict, time_Array, lats, lons, data_type = '2', running_mean_window = 2):
    ...
    if data_type == '1':  # input monthly --> output annually
        ...
        plt.xlabel(' Time ')
        plt.ylabel('SUB '+r'$ [std_{SUB}]$')  # kg*m^{-2}
...
def LWP_obs_trends(data_Array_actual_predict, data_Array_predict_predict, data_Array_actual_training, data_Array_predict_training, time_Array_predict, time_Array_training, lats, lons, data_type = '2', running_mean_window = 2):
    ...
    parameters = {'axes.labelsize': 15, 'legend.fontsize': 13,
              'axes.titlesize': 16, 'xtick.labelsize': 15, 'ytick.labelsize': 15} 
    ...
    if data_type == '1':  # input monthly --> output annually
        ...
        plt.plot(output_time, output_actual, label = 'OBS LWP: ' + '5*5 annually variation', alpha = 1.0, linewidth= 3.40, linestyle = '-', c = 'green', zorder =1)
        plt.plot(output_time, output_predict, label = 'Predi OBS LWP: ' + '5*5 annually variation', alpha = 1.0, linewidth= 2.40, linestyle = '-', c = 'b', zorder =1)
        print(time_Array_predict[0, 0])
        ...
        plt.plot(output_time, output_actual, label = 'OBS LWP: ' + '5*5 monthly variation', alpha = 1.0, linewidth= 3.40, linestyle = '-', c = 'green', zorder =1)
        plt.plot(output_time, output_predict, label = 'Predi OBS LWP: ' + '5*5 monthly variation', alpha = 1.0, linewidth= 2.40, linestyle = '-', c = 'b', zorder =1)
        print(time_Array_predict[0, 0])
        ...
        plt.plot(output_time, output_actual, label = 'OBS LWP: ' + r'$5*5^{o}$' + ' ' + str(12 * running_mean_window + 1) + ' months running mean', alpha = 1.0, linewidth= 3.40, linestyle = '-', c = 'green', zorder = 98)
        plt.plot(output_time, output_predict, label = 'CCFs Predicted LWP: ' + r'$5*5^{o}$' + ' '+ str(12 * running_mean_window + 1) + ' months running mean', alpha = 1.0, linewidth= 2.40, linestyle = '-', c = 'b', zorder = 98)
        ax1.axvline(time_Array_predict.shape[0], linestyle = '--', linewidth = 2.0, c = 'k')
        ...
        # plt.legend()
        # plt.show()
        plt.savefig(path_Plotting + 'Trends_obs_predict+train_2_LWP'+str(running_mean_window)+'yrsrunningvariation.jpg', bbox_inches = 'tight', dpi = 300)
