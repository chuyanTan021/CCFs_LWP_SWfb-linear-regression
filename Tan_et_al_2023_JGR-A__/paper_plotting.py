...
def Fig4_base(s_range, x_range, y_range, deck2 = deck2, deck_nas2 = deck_nas2, path1 = path_data, path6 = path_plot):
    ...
    #.. linear curve fit for regressed d(LWP) and reported d(LWP) for largest_pi_R_2 model
    POPT_2, POCV_2 = curve_fit(target_func, 1000. * Y_metric, 1000. * Z_metric)

    # Calc the R square, plot the fit line:
    pearsonr_2 = pearsonr(1000. * Y_metric, 1000. * Z_metric)[0]
    R_square_2 = calc_r2(1000. * Y_metric, 1000. * Z_metric)
...
def Fig6_base(s_range, x_range, y_range, deck2 = deck2, deck_nas2 = deck_nas2, path1 = path_data, path6 = path_plot):
    # dLWP/ d P-E &	dallpha /dLWP versus mean state LWP (piControl) + Observational Values:
    ...
    a = 1000. * coef_obs[0] * sigmaXi_r2_obs
    print("standardized dLWP / dCCF coefs of Observations: ", a)
    ...
    print("coef_dict for dAlbedo / dLWP of Observations: ", coef_dict_Albedo_training)
    
    
    # Put in the same alpha_cs threshold (0.15) radiative efficiency, the samllest one(0.5/1.0), and the biggest one(0.11).
    coef_Albedo_obs = np.asarray([coef_dict_Albedo_training['15.0'], coef_dict_Albedo_training['50.0'], coef_dict_Albedo_training['11.0']])
    ...
    stcoef_albedo1 = asarray(stcoef_albedo1)/ 1000. #  * sigmaLWP_ALL
    ...
    # Print the GCMs' radiative efficiencies at alpha_cs threshold = 0.15:
    
    for i in range(len(deck_nas2)):
    print(deck2[i]['modn'], 'radiative efficiencies at alpha_cs threshold = 0.15: ', output_2lrm_dict_Albedo[deck_nas2[i]]['15.0'][0])
    ...
    for i in range(len(deck2)):
        ...
        mean_Warm_LWP.append(nanmean(area_mean(rd_Warm['LWP'], y_range, x_range)) + a)
    ...
    Coef_albedo_lwp = asarray(Coef_albedo_lwp).reshape(N) * 100.
    ...
    # GCM's dLWP_dTs:
    # Cold:
    ax33.plot(mean_Cold_LWP, Coef_LWP_p_e[:,0], marker = 's', markersize = 8, c = 'tab:blue', fillstyle = 'none', markeredgewidth = 2., linestyle = 'None')

    # Warm:
    ax33.plot(mean_Warm_LWP, Coef_LWP_p_e[:,1], marker = 's', markersize = 8, c = 'pink', fillstyle = 'none', markeredgewidth = 2., linestyle = 'None')

    ax33.set_xlabel(r"$ piControl\ \overline{LWP}\ $" + r"$[g\ m^{-2}]$ ")
    ax33.set_ylabel(r"$ \partial LWP/\partial {P - E}\ $" + r"$[g\ m^{-2}/\ (mm\ day^{-1})]$ ", c = 'k')  # \ mm^{-1}\ day\ mm^{-1}\ day \ std.dev^{-1}
    ...
    color = 'tab:green'
    ax34.plot(mean_state_LWP, Coef_albedo_lwp, marker = 'o', markersize = 8, c = 'tab:green', linestyle = 'None')
    ax34.tick_params(axis='y', labelcolor=color)
    ax34.set_ylabel(r"$ \partial \alpha/ \partial LWP|\ {January}\ [\% /\ (g\ m^{-2})]$ ", color=color)  # we already handled the x-label with ax1  #  \ std.dev^{-1}(LWP)
    ...
    ax34.errorbar(x= mean_state_LWP_obs, y = (100. / 1000. * asarray(coef_Albedo_obs[0,0])), yerr = yerror, marker = '^'
                  , color = 'tab:green', markersize = 14.8, markeredgecolor = 'black', markeredgewidth = 2.0, capsize = 9, elinewidth = 2.0, zorder = 0)

    # Observations' dLWP_dTs:
    ax33.plot(mean_state_LWP_obs, 1000.* asarray(coef_obs[0][1]), marker = '^'
             , color = 'pink', markersize = 14.8, markeredgecolor = 'black', markeredgewidth = 2.0, linestyle = 'None', zorder = 100)
    ...
    # plt.savefig(path6+"plot_meanstatelwp_LWPqconv_RE_2lrm_RElwp(95)_Nov9th.jpg", bbox_inches='tight', dpi = 425)  # _stdedcoef
    # plt.savefig(path6 + "Fig6.jpg", bbox_inches = 'tight', dpi = 425)
