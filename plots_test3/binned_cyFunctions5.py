...
def binned_cyGlobal_anr(S, lat, lon, r):
    '''
    Calculate the binned array for the mean value within any 'r'esolution degree Bin Boxes in global REGION
    '''

    XX, YY  = np.meshgrid(lon, lat, indexing='xy')
    #..Global region from 90S 90N
    x_range  = np.arange(-180., 183, float(r))   #..number:
    y_range  = np.arange(-90., 90,  float(r))   #..number()
...
def binned_cySouthOcean_anr(S, lat, lon, r):
    '''
    Calculate the binned array for the mean value within 5X5 degree Bin Boxes in SouthOcean REGION
    '''
    ...
    XX, YY  = np.meshgrid(lon, lat, indexing='xy')
    #..Southern Ocean region from 85S 40S
    x_range  = np.arange(-180., 183,  float(r))   #..number:73
    y_range  = np.arange(-85., -35,  float(r))   #.. (10)
...
def binned_cySouthOCEAN_anr(S, lat, lon, r):
    '''
    Calculated the binned array for any  resolution('r'_degree X 'r'_degree) in Southern Ocean Region
    Specfic for data(Mac \   ERA5) whose 'lon' points are from 0. to 360.
    '''
    
    XX, YY  = np.meshgrid(lon, lat, indexing='xy')
    
    #..Southern Ocean region from 85S 40S
    x_range  = np.arange( 0.,  360.001, float(r))   #..number: (360 / r)+1 = ()
    y_range  = np.arange(-86., -39.998, float(r))   #..  (45/ r) +1= ()
    ...
    S_binned_array  = np.zeros((S.shape[0],ybins-1,xbins-1))
    ...
    for i in np.arange(S.shape[0]):
        ...
        ind =  np.isnan(S[i,:,:])==False
        S_binned_time , xedge, yedge, binnumber  = stats.binned_statistic_2d(XX[ind].ravel(),YY[ind].ravel(), values = S_time_step[ind].ravel(),
                                                                            statistic ='mean', bins=[x_range, y_range], expand_binnumbers =True)
        
        S_binned_array[i,:,:] = S_binned_time.T
        
    lat_new = np.arange(-85.5, -40.498, float(r))   #..return as new lat(lon) single-dimension array
    lon_new = np.arange( .5,   359.501, float(r))   

    return S_binned_array, lat_new, lon_new


def binned_skiTamnW(XX,  YY,  S, lat_Y, lon_X):
    '''
    Calculate the binned array for the mean value of LWP over T_skew-Wvp bidimensional axes
    '''
