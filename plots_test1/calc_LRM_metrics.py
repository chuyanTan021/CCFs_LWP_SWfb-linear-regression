...
def calc_LRM_metrics(THRESHOLD_sst, THRESHOLD_sub,**model_data):
    ...
    print('abr4x Eva:', Eva_abr.shape, 'abr4x mean-gmt(K): ', np.nanmean(gmt_abr))
    ...
    WD = '/glade/work/chuyan/Research/Cloud_CCFs_RMs/plots_test1/'
    ...
    rawdata_dict3['Bestfit_sub'] =  THRESHOLD_sub
    savez(WD+C_dict['model_data']['modn']+'mond_'+str(TR_sst)+'_dats', model_data=C_dict['model_data'], rawdata_dict = rawdata_dict3)
    ##  best fit save_2lrm command:
    # savez(WD+C_dict['model_data']['modn']+'_best(test3)fit_'+str(TR_sst)+'_dats', model_data=C_dict['model_data'], rawdata_dict = rawdata_dict3)
    ...
    rawdata_dict4['Bestfit_sst'] =  THRESHOLD_sst
    rawdata_dict4['Bestfit_sub'] =  THRESHOLD_sub
    savez(WD+C_dict['model_data']['modn']+'mond_'+str(TR_sst)+'K_'+ 'ud'+str(TR_sub)+'_dats', model_data=C_dict['model_data'], rawdata_dict = rawdata_dict4)
    ##  best fit save_4lrm command:
    # savez(WD+C_dict['model_data']['modn']+'_best(test3)fit_'+str(TR_sst)+'K_'+ 'ud'+str(round(TR_sub*100,1))+'_dats', model_data=C_dict['model_data'], rawdata_dict = rawdata_dict4)
...
def fitLRM(C_dict, TR_sst, s_range, y_range, x_range):
    ...
    print('2LRM: HERE TR_sst = ', TR_sst, 'K')
    # PI
    ...
    print("2lrm: mean regrssed value:", nanmean(YB), " in 'pi'")
    YB_iwp[ind6]  = sstleiwp_predi
    YB_iwp[ind7] =  sstltiwp_predi

    # 'YB' resample into the shape of 'LWP_yr_bin':
    C_dict['LWP_predi_bin_PI']   =  array(YB).reshape(shape_mon_PI)
    print("Regressed LWP array in 'PI ' exp, shape in :",  C_dict['LWP_predi_bin_PI'].shape)

    C_dict['IWP_predi_bin_PI']  = array(YB_iwp).reshape(shape_mon_PI)
    print("This is regressed IWP array in 'PI' exp, shape in :",  C_dict['IWP_predi_bin_PI'].shape)
    ...
    print('examine the mean of LWP for reported / regred pi-C SST>or= Tr_sst points:', nanmean(dict2_predi_fla_PI['LWP'][ind6]), nanmean(sstlelwp_predi))
    print('examine the mean of LWP for reported / regred pi-C SST< Tr_sst points:', nanmean(dict2_predi_fla_PI['LWP'][ind7]), nanmean(sstltlwp_predi))
    # print('examine the mean of IWP for reported / regred pi-C all points:', mean(dict2_predi_fla_PI['IWP']) , mean(YB_iwp))
    ...
    print("2lrm: regred value in 'abr': ", YB_abr)
    print("2lrm: reported value in 'abr': ", dict2_predi_fla_abr['LWP'])
    ...
    C_dict['LWP_predi_bin_abr']   =  array(YB_abr).reshape(shape_mon_abr)
    C_dict['IWP_predi_bin_abr']  =   array(YB_abr_iwp).reshape(shape_mon_abr)

    print(" Regressed LWP array in abr exp, shape in :",  C_dict['LWP_predi_bin_abr'].shape)   
...
def fitLRM2(C_dict, TR_sst, TR_sub, s_range, y_range, x_range):
    ...
    print('4LRM: HERE TR_sst = ', TR_sst, 'K')
    ...
    print("4lrm: mean regrssed value:", nanmean(YB), " in 'pi'")
    ...
    print("4lrm: regrssed value:", YB_abr, " in 'abr'")
