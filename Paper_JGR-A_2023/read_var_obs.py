...
def read_var_obs_CERES(varnm = 'toa_sw_all_mon', valid_range1 = [2002, 7, 15], valid_range2 = [2016, 12, 31]):
    ...
    # 'varnm' could be 
    # 1. 'toa_sw_all_mon': Top of The Atmosphere Shortwave flux in all-sky (Up), monthly mean value; 
    # 2. 'toa_lw_all_mon': Top of The Atmos Longwave flux in all-sky (Up), monthly mean value;
    # 3. 'toa_net_all_mon': Top of The Atmos Net flux in all-sky (Down): = 4 - (1 + 2);
    # 4. 'solar_mon': Top of The Atmos incoming Shortwave flux (Down), monthly mean value;
    # 5. 'toa_sw_clr_t_mon': Top of The Atmosphere Shortwave Flux, Clear-Sky (for total region) conditions, Monthly Means
    # 6. 'toa_lw_clr_t_mon': Top of The Atmosphere Longwave Flux, Clear-Sky (for total region) conditions, Monthly Means
    # 7. 'toa_net_clr_t_mon': Top of The Atmosphere Net Flux in Clear-Sky (for total region) conditions, Monthly Means: = 4 - (5 + 6);
    # valid_range1 & 2: the starting and end time_stamps of time sequential metric. Only first two numbers are valid.
    ### ---------------
    
    folder = pp_path_OBS
    
    # fn = glob.glob(folder+"CERES_EBAF-TOA_Ed4.1_Subset_200207-202203.nc")  # Clear-sky fluxes only for cloud-free portion of the gridbox
    fn = glob.glob(folder+"CERES_EBAF_Ed4.1_Subset_200207-202203.nc")  # Clesr-sky fluxes correctted for the totol gridbox
