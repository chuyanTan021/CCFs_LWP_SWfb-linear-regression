...
def calc_Radiation_LRM_1(inputVar_pi, inputVar_abr, TR_albedo = 0.15):
    ...
    # 5^o * 5^o binned Array:
    # Monthly variables (same as above):
    dict1_mon_bin_PI = {}
    dict1_mon_bin_abr = {}
    
    # binned lat, lon_array:
    ...
    for c in range(len(datavar_nas)):
        dict1_mon_bin_abr[datavar_nas[c]+'_mon_bin'] = binned_cySouthOcean5(dict1_abr_var[datavar_nas[c]][:], lats[latsi0: latsi1+1], lons)
        dict1_mon_bin_PI[datavar_nas[c]+'_mon_bin'] = binned_cySouthOcean5(dict1_PI_var[datavar_nas[c]][:], lats[latsi0: latsi1+1], lons)

        dict1_mon_bin_abr[datavar_nas[c]+'_delta_mon_bin'] = dict1_mon_bin_abr[datavar_nas[c]+'_mon_bin'] - nanmean(area_mean( dict1_mon_bin_abr[datavar_nas[c]+'_mon_bin'], y_range, x_range) )
        dict1_mon_bin_PI[datavar_nas[c]+'_delta_mon_bin'] = dict1_mon_bin_PI[datavar_nas[c]+'_mon_bin'] - nanmean(area_mean( dict1_mon_bin_PI[datavar_nas[c]+'_mon_bin'][:], y_range, x_range) )
    
    # radiative transfer model: single regime LRM:

    # training (PI):

    x_pi = dict1_mon_bin_PI['LWP_delta_mon_bin'].flatten()
    
    y_pi = dict1_mon_bin_PI['alpha_cre_delta_mon_bin'].flatten()
    
    y2_pi = dict1_mon_bin_PI['albedo_delta_mon_bin'].flatten()
    
    cs_pi = dict1_mon_bin_PI['albedo_cs_delta_mon_bin'].flatten()
    print("TR_albedo_cs = ", TR_albedo, ".")

    # rule out Land and Sea Ice: albedo_cs< = threshold_alpha:
    ind_nolsi_pi = cs_pi <= TR_albedo
    print("albedo < "+ str(TR_albedo), " ratio in 'piControl': "+str(asarray(nonzero(ind_nolsi_pi == True)).shape[1] / ind_nolsi_pi.shape[0]))
    ...
    x_abr = dict1_mon_bin_abr['LWP_delta_mon_bin'].flatten()
    
    y_abr = dict1_mon_bin_abr['alpha_cre_delta_mon_bin'].flatten()
    
    y2_abr = dict1_mon_bin_abr['albedo_delta_mon_bin'].flatten()
    
    cs_abr = dict1_mon_bin_abr['albedo_cs_delta_mon_bin'].flatten()
    ...
    # print the summary
    print("model1, alpha_cre = a1 * lwp + A2: ", ' ',model1_abr.summary())
    print("model2, albedo = a1* lwp + a2 * albedo_cs + A3: ", ' ',model2_abr.summary())
