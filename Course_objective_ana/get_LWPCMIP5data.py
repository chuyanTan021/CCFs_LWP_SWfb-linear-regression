...
def get_LWPCMIP5(modn='', consort='', cmip='', exper='', ensmem='', typevar='Amon'):
    #if exper == 'historical':
    #    amip = False
    #else:
    #    amip = True
    ...
    sfc_T_abr        = read_var_mod(modn=modn, consort=consort, varnm='ts', cmip=cmip, exper= exper, ensmem=ensmem, typevar=typevar, read_p=False, time1= time1, time2= time2)[0]

    T_700_alevs_abr,Pres_abr,lat_abr,lon_abr,times_abr = read_var_mod(modn=modn, consort=consort, varnm='ta', cmip=cmip, exper= exper, ensmem=ensmem, typevar=typevar, read_p=True, time1= time1, time2= time2)
    T_700_abr = T_700_alevs_abr[:, 3,:,:]   #..700 hPa levels

    sfc_P_abr       = read_var_mod(modn=modn, consort=consort, varnm='ps', cmip=cmip, exper= exper, ensmem=ensmem, typevar=typevar, read_p=False, time1= time1, time2= time2)[0]

    sub_abr_alevs =  read_var_mod(modn=modn, consort=consort, varnm='wap', cmip=cmip, exper= exper, ensmem=ensmem, typevar=typevar, read_p=True, time1= time1, time2= time2)[0]
    sub_abr          =  sub_abr_alevs[:, 5,:,:]
    #..500mb downward motions

    clivi_abr       = read_var_mod(modn=modn, consort=consort, varnm='clivi', cmip=cmip, exper= exper, ensmem=ensmem, typevar=typevar, read_p=False, time1= time1, time2= time2)[0]
    clwvi_abr       = read_var_mod(modn=modn, consort=consort, varnm='clwvi', cmip=cmip, exper= exper, ensmem=ensmem, typevar=typevar, read_p=False, time1= time1, time2= time2)[0]
    ...
    rsutcs_abr      = read_var_mod(modn=modn, consort=consort, varnm='rsutcs', cmip= cmip, exper=exper, ensmem=ensmem, typevar=typevar, read_p=False, time1 = time1, time2= time2)[0]
    ...
    sfc_T_pi       = read_var_mod(modn= modn, consort= consort, varnm='ts', cmip=cmip, exper= exper, ensmem=ensmem, typevar=typevar, read_p=False, time1= timep1, time2= timep2)[0]

    T_700_alevs,Pres_pi,lat_pi,lon_pi,times_pi =  read_var_mod(modn= modn, consort= consort, varnm='ta', cmip=cmip, exper= exper, ensmem=ensmem, typevar=typevar, read_p=True, time1= timep1, time2= timep2)
    T_700_pi       = T_700_alevs[:, 3,:,:]

    sfc_P_pi       = read_var_mod(modn= modn, consort= consort, varnm='ps', cmip=cmip, exper= exper, ensmem=ensmem, typevar=typevar, read_p=False, time1= timep1, time2= timep2)[0]   
    #..sea surface Pressure, Units in Pa

    sub_alevs      = read_var_mod(modn= modn, consort= consort, varnm='wap', cmip=cmip, exper= exper, ensmem=ensmem, typevar=typevar, read_p=True, time1= timep1, time2= timep2)[0]
    sub_pi         =  sub_alevs[:, 5,:,:]
    #..500mb downward motion

    clivi_pi       = read_var_mod(modn= modn, consort= consort, varnm='clivi', cmip=cmip, exper= exper, ensmem=ensmem, typevar=typevar, read_p=False, time1= timep1, time2= timep2)[0]
    #..ICE WATER PATH, Units in kg m^-2
    clwvi_pi       = read_var_mod(modn= modn, consort= consort, varnm='clwvi', cmip=cmip, exper= exper, ensmem=ensmem, typevar=typevar, read_p=False, time1= timep1, time2= timep2)[0]
    
    tas_pi         = read_var_mod(modn= modn, consort= consort, varnm='tas', cmip=cmip, exper= exper, ensmem=ensmem, typevar=typevar, read_p=False, time1= timep1, time2= timep2)[0]
    ...
    rsutcs_pi      = read_var_mod(modn=modn, consort=consort, varnm='rsutcs', cmip= cmip, exper=exper, ensmem=ensmem, typevar=typevar, read_p=False, time1 = timep1, time2= timep2)[0]
